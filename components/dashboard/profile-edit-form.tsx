...
export function ProfileEditForm({ initialName, initialImage, email }: ProfileEditFormProps) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-border bg-background p-5">
      <div>
        <h3 className="text-lg font-semibold text-foreground">프로필 수정</h3>
        <p className="mt-1 text-sm text-muted">회원 정보를 업데이트합니다.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="space-y-2">
          <span className="text-sm text-muted">프로필 이미지 URL (선택)</span>
          <input
            type="url"
            value={image}
      ...
      <div className="text-sm text-muted">이메일: {email ?? "이메일 정보 없음"}</div>

      {error ? <p className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-500">{error}</p> : null}
      {success ? <p className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-500">{success}</p> : null}

      <button
        type="submit"
        disabled={isPending || !isDirty}
        className="inline-flex items-center justify-center rounded-xl border border-accent/60 px-4 py-2 text-sm font-semibold text-accent transition hover:border-accent-dark hover:bg-accent-soft disabled:cursor-not-allowed disabled:opacity-50"
      >
